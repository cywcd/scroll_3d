...
*/
	...
	sigeUpPopupOpen : function(){
		...
		$('.u-sigeUp').on('click',function(){
			...
			if(sw=='false' && !$('#check_img').val()){   //报名时间结束后如果已报名可以查看二维码
				return;
			};
			...
			if(sigeUp.length>=1) {
				...
				// 报名未成功弹出报名
					...
					if($('#fakeid').val()){
						//报名统计请求
						ylLightAPP14.ajaxTongji(dataLayout,dataName);
					...
					// 不是微信弹出微信扫描
						...
						if(ylLightAPP14.transformNode.attr('id')=='transformNode') $('.erweima').addClass('m-popup');
						$('.erweima').css('display','block')
						setTimeout(function(){
							$('.erweima').addClass('show');
							$(window).on('webkitTransitionEnd transitionend',popupOpen);

							function popupOpen(){
								var imgSrc = $('#qr_img').val();
								$('.erweima .erweima-content').addClass('show');
								$('.erweima .erweima-content').find('.img img').attr('src',imgSrc);

								//提示图片的保存方式
								$('.erweima .erweima-content').find('.saveImg a').on('click',ylLightAPP14.qrPopup);
								$('.erweima .erweima-content').find('.arrow').on('click',ylLightAPP14.qrPopup);
								$('.erweima .erweima-content').on('click',function(e){e.stopPropagation()});
								$('.erweima').on('click',ylLightAPP14.qrPopup);
		...
		$.ajax({
			...
			success: function(msg){
				...
				if(msg.code=='200'){   
					...
					var fail_msg = $('.popup_error').eq(0).text();
					if(!fail_msg){
						fail_msg = '链接已失效，请重新扫描二维码!';
					}
					$('.popup_error').html(fail_msg);
					$(".popup_sucess").removeClass("on");
					$(".popup_error").addClass("on");
				}
				setTimeout(function(){
...
*/
	...
	pluin : function(){
		...
		//赞助商轮播图加载
		$(".partnerBox").each(function(){
			var urls = $(this).find('input').val(),
				ulImg = $(this).find('ul'),
				lpos = $(this).find('li').size();
			new slidepic(this,{urls:urls,ulImg:ulImg,lpos:lpos,posInit:posInit,posSet:posSet});
		});	
